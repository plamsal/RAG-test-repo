...
Location information (city and state) is encapsulated in a separate `Location` class, which the Job class has a composition relationship with (indicated by the filled diamond and 1-to-1 multiplicity). This design decision separates location concerns and makes the system more maintainable if additional location attributes are needed in the future. The Job class includes an `isCurrentJob` boolean attribute to distinguish the current job from job offers when displaying ranked lists.
...
### Location Class
Location information (city and state) is encapsulated in a separate class rather than storing these fields directly in the Job class. This design decision improves modularity and makes the system more maintainable if additional location attributes (such as country or postal code) need to be added in the future. The composition relationship between Job and Location ensures that each job has exactly one location.

### ComparisonResult Class
Comparison outputs are encapsulated in a dedicated ComparisonResult class to separate the concerns of data calculation (performed by JobComparator) from data presentation (handled by the GUI layer). This class packages the two jobs being compared along with their calculated scores, and provides methods to retrieve formatted display data via `getJob1Data()` and `getJob2Data()`.

### JobComparator Class
JobComparator is designed as a stateless utility class containing only static methods for scoring, ranking, and comparison operations. This design separates calculation logic from data storage, improves testability by isolating the scoring algorithm, and makes the comparison logic reusable across multiple operations (ranking and individual comparisons). The class has no attributes, following the utility class pattern.
